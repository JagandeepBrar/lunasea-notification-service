 ...
 */
export const bufferWarningPayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    ...
    const body = !payload?.data?.message?.length ? `${user} (${player}) is buffering ${title}` : payload?.data?.message;
 ...
 */
export const playbackErrorPayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const user = payload?.data?.user ?? 'Unknown User';
    const player = payload?.data?.player ?? 'Unknown Player';
    const title = payload?.data?.title ?? 'Unknown Content';
 ...
 */
export const playbackPausePayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const user = payload?.data?.user ?? 'Unknown User';
    const player = payload?.data?.player ?? 'Unknown Player';
    const title = payload?.data?.title ?? 'Unknown Content';
    const body = !payload?.data?.message?.length ? `${user} (${player}) has paused ${title}` : payload?.data?.message;
    return <NotificationPayload>{
 ...
 */
export const playbackResumePayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const user = payload?.data?.user ?? 'Unknown User';
    const player = payload?.data?.player ?? 'Unknown Player';
    const title = payload?.data?.title ?? 'Unknown Content';
    const body = !payload?.data?.message?.length ? `${user} (${player}) has resumed ${title}` : payload?.data?.message;
    return <NotificationPayload>{
 ...
 */
export const playbackStartPayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const user = payload?.data?.user ?? 'Unknown User';
    const player = payload?.data?.player ?? 'Unknown Player';
    const title = payload?.data?.title ?? 'Unknown Content';
    const body = !payload?.data?.message?.length ? `${user} (${player}) started playing ${title}` : payload?.data?.message;
    return <NotificationPayload>{
 ...
 */
export const playbackStopPayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const user = payload?.data?.user ?? 'Unknown User';
    const player = payload?.data?.player ?? 'Unknown Player';
    const title = payload?.data?.title ?? 'Unknown Content';
    const body = !payload?.data?.message?.length ? `${user} (${player}) has stopped ${title}` : payload?.data?.message;
    return <NotificationPayload>{
 ...
 */
export const plexRemoteAccessBackUpPayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const body = !payload?.data?.message?.length ? 'The Plex Media Server remote access is back up' : payload?.data?.message;
    return <NotificationPayload>{
 ...
 */
export const plexRemoteAccessDownPayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const reason = payload?.data?.remote_access_reason ?? 'Unknown Reason';
    const body = !payload?.data?.message?.length ? `The Plex Media Server remote access is down (${reason})` : payload?.data?.message;
    return <NotificationPayload>{
 ...
 */
export const plexServerBackUpPayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const body = !payload?.data?.message?.length ? 'The Plex Media Server is back up' : payload?.data?.message;
    return <NotificationPayload>{
 ...
 */
export const plexServerDownPayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const body = !payload?.data?.message?.length ? 'The Plex Media Server is down' : payload?.data?.message;
    return <NotificationPayload>{
 ...
 */
export const plexUpdateAvailablePayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
 ...
 */
export const recentlyAddedPayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const title = payload?.data?.title ?? 'Unknown Content';
    const body = !payload?.data?.message?.length ? `${title} was recently added to Plex` : payload?.data?.message;
    return <NotificationPayload>{
 ...
 */
export const tautulliUpdateAvailablePayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const version = payload?.data?.tautulli_update_version ?? 'Unknown Version';
    const body = !payload?.data?.message?.length ? `An update is available for Tautulli (version ${version})` : payload?.data?.message;
    return <NotificationPayload>{
 ...
 */
export const testPayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    return <NotificationPayload>{
 ...
 */
export const transcodeDecisionChangePayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const user = payload?.data?.user ?? 'Unknown User';
    const player = payload?.data?.player ?? 'Unknown Player';
    const title = payload?.data?.title ?? 'Unknown Content';
    const body = !payload?.data?.message?.length
 ...
 */
export const userConcurrentStreamsPayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const user = payload?.data?.user ?? 'Unknown User';
    const streams = payload?.data?.user_streams ?? '??';
    const body = !payload?.data?.message?.length ? `${user} has ${streams} concurrent streams` : payload?.data?.message;
    return <NotificationPayload>{
 ...
 * Construct a NotificationPayload based on a user concurrent streams event.
 */
export const userNewDevicePayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const user = payload?.data?.user ?? 'Unknown User';
    const player = payload?.data?.player ?? 'Unknown Player';
    const body = !payload?.data?.message?.length ? `${user} is streaming from a new device: ${player}` : payload?.data?.message;
    return <NotificationPayload>{
 ...
 */
export const watchedPayload = async (payload: Models.ActionTypePayload, profile: string): Promise<NotificationPayload> => {
    const user = payload?.data?.user ?? 'Unknown User';
    const player = payload?.data?.player ?? 'Unknown Player';
    const title = payload?.data?.title ?? 'Unknown Content';
    const body = !payload?.data?.message?.length ? `${user} (${player}) has watched ${title}` : payload?.data?.message;
    return <NotificationPayload>{
 ...
 * Construct a NotificationPayload based on a watched event.
 */
export const watchedPayloadDeprecated = async (data: Models.WatchedEventTypeDeprecated, profile: string): Promise<NotificationPayload> => {
    const user = data.user ?? 'Unknown User';
    const player = data.player ?? 'Unknown Player';
    const title = data.title ?? 'Unknown Content';
