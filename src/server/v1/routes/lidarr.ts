...
/**
 * Route namespace to contain all Lidarr related routes and handlers.
 */
export namespace Route {
    ...
    /**
     * Lidarr User Handler: Handles a webhook from Lidarr, and sends a notification to all devices that are attached to the calling account.
     *
     * @param request Express request object
     * @param response Express response object
     */
    async function userHandler(request: express.Request, response: express.Response): Promise<void> {
        Logger.info('Running Lidarr [user] webhook...');
        try {
        ...
        Logger.info('Finished Lidarr [user] webhook.');
    }

    /**
     * Lidarr Device Handler: Handles a webhook from Lidarr [device ID], and sends a notification to the single device.
     *
     * @param request Express request object
     * @param response Express response object
     */
    async function deviceHandler(request: express.Request, response: express.Response): Promise<void> {
        Logger.info('Running Lidarr [device] webhook...');
        try {
            Logger.debug('-> Sending HTTP response to complete webhook...');
            response.status(200).json(<Server.Response>{ message: Constants.MSG_OK });
            Logger.debug('-> HTTP response sent (200 OK)');
        ...
        Logger.info('Finished Lidarr [device] webhook.');
    }
}
