...
export async function extractDeviceToken(
  ...
  if (request.params.id) {
    ...
    response.status(400).json(<Models.Response>{ message: Constants.MSG_NO_ID_SUPPLIED });
...
export async function pullUserTokens(
  ...
  if (deviceCount > 0) {
    ...
    Logger.debug('->', deviceCount, 'device(s) found');
    Logger.debug('Finished pullUserTokens middleware.');
    next();
  } else {
    Logger.warn('-> 0 device(s) found. Cancelling request...');
    Logger.debug('-> Sending HTTP response to complete webhook...');
    response.status(400).json(<Models.Response>{ message: Constants.MSG_NO_DEVICES_FOUND });
    Logger.debug('-> HTTP response sent (400 Bad Request)');
    Logger.debug('Finished pullUserTokens middleware.');
  }
}
...
export async function validateUser(
  ...
  if (request.params.id && (await Firebase.validateUserID(request.params.id))) {
    ...
    response.status(404).json(<Models.Response>{ message: Constants.MSG_USER_NOT_FOUND });
